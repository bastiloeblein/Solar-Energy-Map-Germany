  ...
  if (geotiff_gray_layer != null) {
    try {
      ...
      coordinateOutput.innerText +=
        ",  Strahlungswerte (in kWh/m²) an der ausgewählten Position: " + value[0];
    } catch {
      coordinateOutput.innerText = `EPSG-3857-Coordinates: ${roundedCoordinates.join(", ")}`;
      coordinateOutput.innerText +=
        ",  Strahlungswerte (in kWh/m²) an der ausgewählten Position: n.A.";
    }
  } else {
    coordinateOutput.innerText = `EPSG-3857-Koordinaten: ${roundedCoordinates.join(", ")}`;
  }
  ...
  return fetch("http://127.0.0.1:5000/load_Solarmodules_for_selected_regions", {
    ...
    .then((data) => {
        if (data.features && data.features.length === 0) {
          // Leere Feature-Liste, zeige Popup an
          alert("Für den gesuchten Bereich haben wir leider keine Anlagen gefunden");
        } else
 {        // Daten erfolgreich geladen, jetzt zur Karte hinzufügen
...
document.getElementById("dwd").addEventListener("click", function (event) {
  event.preventDefault();
   ...
   // Remove existing scale image if present
   const existingScale = document.getElementById("scaleImage");
   if (existingScale) {
     existingScale.remove();
   }

   // Add scale image (skala.jpg) to the right side of the screen
  const scaleImage = document.createElement("img");
  scaleImage.id = "scaleImage";
  scaleImage.src = "skala.jpg";  // Update the path to the actual image location
  
  // Responsive styles
  scaleImage.style.position = "fixed";
  scaleImage.style.right = "2%";  // Align 2% from the right edge of the screen
  scaleImage.style.bottom = "25%";  // Some margin (5%) from the bottom
  scaleImage.style.height = "40vh";  // Height will be 40% of the viewport height
  scaleImage.style.width = "auto";  // Maintain aspect ratio
  scaleImage.style.zIndex = "1000";  // Ensure it appears on top
  scaleImage.style.maxHeight = "80vh";  // Max height is 80% of the viewport height
  scaleImage.style.maxWidth = "15vw";  // Max width is 15% of the viewport width
...
document.getElementById("hideDWD").addEventListener("click", function (event) {
  ...
  // Remove the scale image if present
  const existingScale = document.getElementById("scaleImage");
  if (existingScale) {
    existingScale.remove();
  }
